...
3. **USP<71> SLA tier — RESOLVED (Handler, 2026-07-01):** ~14-day target turnaround confirmed for its per-service tier.
...
8. **USP<71> COA timing (Full B / amendments) — RESOLVED (Handler, 2026-07-01):** issue PCR results first as a **preliminary** certificate, then **re-publish (amend)** when USP<71> completes — the lab's established practice. This must produce a **traceable amendment** (7.5.2), not a silent overwrite. Drives the native amendment flow, which lands in the coabuilder/COA plan (after the Accu-Mk1 intake plan), not in the intake slice.
