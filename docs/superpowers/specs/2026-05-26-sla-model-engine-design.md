...
4. **Verify the join key:** ~~confirm `SamplePriority.sample_uid` matches the `senaite_id` used in `sampleLookupMap` / `getOrderReceivedAt`.~~ **RESOLVED (2026-05-26):** they do **NOT** match directly. `senaite_id` (the `sampleLookupMap` key, passed to `lookupSenaiteSample`) is the **human-readable sample ID** (e.g. `PB-0056`; the backend lookup uppercases it and queries SENAITE by `getId`/`getRequestID`). `SamplePriority.sample_uid` is the **32-char SENAITE UID** (`item.get("uid")`, written by `PUT /worksheets/inbox/{sample_uid}/priority`). **Bridge for D2:** the `SenaiteLookupResult` already cached under each `senaite_id` carries both `sample_id` *and* `sample_uid`, so D2 resolves per-sample priority via `senaite_id → lookup.data.sample_uid → SamplePriority.priority` — no extra backend round-trip, just one hop before `resolveSlaTarget`. (Priority is sparse anyway — absent a `SamplePriority` row, default to `'normal'`, which resolves to the default SLA per note 1.)
