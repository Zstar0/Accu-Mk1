...
4. Cancel ships in this deploy and works in both modes; in senaite mode a cancel that SENAITE refuses (anything after the first worksheet assignment) moves `native_status` and cancels the rows but the badge follows SENAITE until the flip (the dialog says so).
5. Later slice: IS notification on cancel; reinstate; retiring the SENAITE-sourced writers once SENAITE is disconnected.

### Operator notes (from the final whole-branch review)
- **Expect `publish` retry rows from day one** on partial publishes: `_after_publish_native` enqueues whenever SENAITE's read-back is not `published`. They resolve as `superseded` on the first tick when native is `waiting_for_addon_results`; a sample whose native cascade reached `published` while SENAITE parks in `waiting_for_addon_results` retries for ~2 days and then surfaces as `senaite_tee_gave_up`. Watch `senaite_lagging` in `GET /api/workflow/shadow/summary` during the dark period and treat the first batch as spec §9 residual work, not a regression.
- **One new SENAITE write in senaite mode:** when the native cascade reaches `verified`, the branch now tees `verify` at the AR level (the PB-0462 fix, spec §5). Tell the lab: the badge is still SENAITE's, but SENAITE will start reaching Verified on its own for the miscount class.
- **Native-born samples (no SENAITE uid)** never mint tee retry rows; if `SENAITE_URL` is ever unset while samples still carry SENAITE uids, the retry job will churn until the disconnect slice gates the tee on SENAITE being configured.
- **Dev DB hygiene:** `tests/test_workflow_engine.py` runs against the live dev Postgres; keep the dev DB's `registry_read_source.sample_status` at `senaite` (or absent) or those tests write real status/ledger rows.
