...
- **SENAITE tee with read-back and retry**: verify / publish / cancel are teed to SENAITE, proven by re-reading the AR (SENAITE answers 200 to refused transitions), and refusals are queued in `lims_senaite_tee_retries` for the `senaite_tee_retry` job (5 min; backoff 5 → 720 min; gives up after 8 attempts). A refused publish issues `verify` first (the PB-0462 "stuck To Verify" class). Cancel after verification is recorded as `senaite_only`. The shadow summary reports `senaite_lagging`.
