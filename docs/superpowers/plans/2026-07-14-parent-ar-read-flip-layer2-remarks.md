...
> **As-built correction (2026-07-14, final review):** the "two sites" count above was incomplete. `update_senaite_sample_fields` (`POST /wizard/senaite/samples/{uid}/update`, `main.py`) is a generic field-update proxy, and both FE remark-entry forms (`AddRemarkForm` in `SampleDetails.tsx`, `AssignRemarksBlock` in `AssignStep.tsx`) route their Remarks write through it via `updateSenaiteSampleFields(uid, { Remarks })` — a third, unflipped SENAITE Remarks write path that the original layer-2 pass missed. Closed at review: the endpoint now pops `Remarks` before forwarding, writes it natively to `lims_sample_remarks` (resolving the sample via `LimsSample.external_lims_uid`, hard-failing if no registry row matches — same fail-closed posture as the receive flow), and forwards only the remaining fields to SENAITE, if any. See `backend/tests/test_update_fields_remarks_intercept.py`.
