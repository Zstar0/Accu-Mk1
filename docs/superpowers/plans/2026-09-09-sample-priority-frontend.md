...
describe('PriorityGlyph', () => {
  it('renders nothing for the default priority', async () => {
    const { container } = wrap(<PriorityGlyph priority={{ key: 'default', rank: 0, source_level: 'default', source_id: null }} />)
  ...
  return (
    ...
    >
      ...
      <SelectContent>
        ...
        {(active ?? []).filter(p => !p.is_default || explicitKey === p.key || true).map(p => (
