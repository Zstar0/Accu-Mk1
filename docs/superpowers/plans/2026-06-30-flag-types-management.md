...
- [ ] **Step 3:** Add a `useFlagTypesMap()` hook (returns `Record<string, FlagTypeDef>` built from `useFlagTypes({})`), with the static `FLAG_TYPES` (flag-catalog.ts) as `initialData`/fallback so it is never empty. Keep static `FLAG_TYPES`/`FLAG_TYPE_ORDER`/`flagTypeDef` as the fallback seed.
...
- [ ] **Step 2:** **FlagsHeaderButton** (count chips :6,45,46): drive chip color + order from `useFlagTypes({})` (active, sorted) instead of the static order; keep counts from `summary.by_type`.
