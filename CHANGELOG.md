...
- **Run status and parameters:** Plate made / Ran on QuantStudio tick every row at once and only ever set, like the endo sheet's tick-all headings (Ran records the QuantStudio on each row); overage, curve, plate type and the order-sort switch are saved on the worksheet.
