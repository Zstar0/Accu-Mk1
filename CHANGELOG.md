...
- **Run status and parameters:** Plate made / Ran on QuantStudio tick every row at once and only ever set, like the endo sheet's tick-all headings; each sample row also has its own Made and Ran ticks, which set and clear, as on the endo table (Ran records the QuantStudio on the row); overage, curve, plate type and the order-sort switch are saved on the worksheet.
...
### Added
- **Customer detail: a Products toggle on the Orders card.** When on, each sample card shows its ordered-product chips (HPLC, Rapid Sterility, Endotoxin, Variance HPLC and so on), the same chips the Order Status page shows, read from the order payload already loaded (no extra requests). Off by default; the choice is remembered per browser (#239).
