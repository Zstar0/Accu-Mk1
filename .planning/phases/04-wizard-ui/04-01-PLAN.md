...
- `listWizardSessions(params?: { status?: string, peptide_id?: number, limit?: number, offset?: number }): Promise<WizardSessionListItem[]>` — GET /wizard/sessions (returns a flat array, NOT a paginated envelope)
...
interface WizardStoreState {
  ...
  // Derived (not stored as function — use stepStates field directly)
  canAdvance: () => boolean
}
```

CRITICAL STORE DESIGN — `stepStates` is a stored FIELD, not a computed function:
- Store `stepStates: Record<StepId, StepState>` as a regular state field initialized to `deriveStepStates(null, 1)`
- Export `deriveStepStates` as a pure function (used internally by the store actions)
- Inside `startSession`: call `set({ session, stepStates: deriveStepStates(session, get().currentStep) })`
- Inside `updateSession`: call `set({ session, stepStates: deriveStepStates(session, get().currentStep) })`
- Inside `setCurrentStep`: call `set({ currentStep: step, stepStates: deriveStepStates(get().session, step) })`
- Inside `resetWizard`: call `set({ session: null, currentStep: 1, stepStates: deriveStepStates(null, 1), loading: false, error: null })`
- Do NOT add a `getStepStates()` function to the store interface — consumers read `stepStates` directly via selector

This ensures `stepStates` is a stable Zustand field. Components read it via `useWizardStore(state => state.stepStates)` which returns a stable reference (only changes when `set()` provides a new object), preventing render cascades.
...
Navigation logic:
- `setCurrentStep`: only allow if target step state is not `locked` (check via `deriveStepStates` or current `stepStates`)
- `canAdvance`: returns true if current step is NOT step 5 AND step `currentStep + 1` is not `locked`
...
- `import { createWizardSession, getWizardSession, recordWizardMeasurement, updateWizardSession, completeWizardSession, listWizardSessions } from '@/lib/api'` resolves
  ...
  <done>
- wizard-store.ts exports useWizardStore with session state, step navigation, and stepStates as a stored field kept in sync by deriveStepStates
- api.ts has 6 wizard API functions and 3 TypeScript interfaces matching backend schemas
- listWizardSessions returns Promise<WizardSessionListItem[]> (flat array)
...
- Read step states from `useWizardStore(state => state.stepStates)` — this is a stored field, stable reference, safe for selectors
...
- Back: `<Button variant="outline" onClick={() => setCurrentStep(currentStep - 1 as StepId)} disabled={currentStep === 1}>Back</Button>`
- Next: `<Button onClick={() => setCurrentStep((currentStep + 1) as StepId)} disabled={currentStep === 5 || !canAdvance()}>Next Step</Button>`
- The Next button disabled condition MUST include `currentStep === 5` as an upper bound guard. Without it, clicking Next on Step 5 would attempt `setCurrentStep(6)` which is not a valid `StepId` (1|2|3|4|5).
...
- WizardStepList renders 5 steps with correct 4-state visual indicators, reading from stepStates field
- WizardStepPanel wraps content with directional slide animations on step change
- Back/Next buttons navigate between steps respecting lock states, Next is disabled on Step 5
...
- Next button is also disabled on Step 5 (upper bound guard prevents invalid StepId)
</verification>

<success_criteria>
- Wizard store manages session state with stepStates as a stored field (no render cascades)
- API layer has all 6 wizard endpoint functions with correct TypeScript types
- listWizardSessions returns a flat array (Promise<WizardSessionListItem[]>)
...
- Next button disabled on Step 5 to prevent invalid StepId(6)
