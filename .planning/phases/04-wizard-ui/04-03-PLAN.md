...
Fetches completed wizard sessions from `listWizardSessions({ status: 'completed', limit: 50 })`.
...
Wrap the existing content in a `Tabs` component (from `@/components/ui/tabs`):

```tsx
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/components/ui/tabs'
import { WizardSessionHistory } from './wizard/WizardSessionHistory'

// In the component, wrap existing list/detail view:
return (
  <div className="flex flex-col gap-4 p-6">
    <Tabs defaultValue="hplc-import">
      ...
      <TabsContent value="hplc-import">
        {/* Existing AnalysisHistory content (search, table, pagination, detail view) */}
      </TabsContent>
...
Move the existing list/detail rendering logic (everything currently in the return) into the "hplc-import" TabsContent. The outer `<div className="flex flex-col gap-4 p-6">` becomes the Tabs wrapper.

Keep the detail view (selectedId != null branch) inside the hplc-import tab — it only applies to HPLC analyses.

Preserve all existing functionality: search, pagination, delete, detail view — just wrap it in a tab.

CRITICAL: Do NOT change the existing AnalysisHistory logic — only wrap it in Tabs and add the new tab. The existing search/pagination/detail states remain local to the component.
  </action>
  <verify>
- `npx tsc --noEmit` passes
- `npm run build` succeeds
