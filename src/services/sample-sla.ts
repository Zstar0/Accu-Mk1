...
export interface SampleSlaResult {
   ...
   *  lookup, no received date, already published), tier resolution failed,
   *  or the `/sla/status` round-trip hasn't returned yet. */
  snapshot: SampleSlaSnapshot | null
  /** Reason snapshot — populated even when `snapshot` is null IF tier
   *  resolution actually ran (lookup present + not published + received).
   *  Lets diagnostic surfaces explain "no tier configured" cases. */
...
export function useSampleSla(
  ...
  // published or unreceived samples.
  ...
  // 1-element array. Hash by uid+target+business+received for cache reuse.
  ...
  }, [applicable, lookup, resolved.tier])
  ...
  return useMemo<SampleSlaResult>(() => {
    ...
    if (!tier) {
      return { snapshot: null, reason, priority, isLoading, isError }
    }
