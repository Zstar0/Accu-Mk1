...
/** Per-user Slack DM notification prefs (spec 2026-07-02). Lives on the
 *  Account → Profile page — it's personal config, not admin settings (the
 *  settings dialog will eventually be admin-only). Server-stored — the
 *  backend notifier is the consumer. Toggles save on change. */
...
export function SlackPrefsSection() {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>{t('preferences.slack.title')}</CardTitle>
        <CardDescription>{t('preferences.slack.blurb')}</CardDescription>
      </CardHeader>
