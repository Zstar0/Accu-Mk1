...
export function WizardSidebar({ vials, activeSampleId, onSelect }: WizardSidebarProps) {
  ...
  return (
    <aside className="border-r bg-muted/20 p-3 overflow-y-auto h-full flex flex-col">
      ...
      <ul className="space-y-1 flex-1">
        {vials.length === 0 && (
          <li className="text-xs text-muted-foreground px-2 py-1">No vials received yet.</li>
        )}
        {vials.map(v => {
